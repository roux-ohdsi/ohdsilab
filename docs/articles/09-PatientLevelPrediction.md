...
<https://ohdsi.github.io/PatientLevelPrediction/>. Researchers can use
this package to predict risk of specific outcomes for persons in a
target cohort. The prediction results can be analyzed programmatically
or in an R Shiny application.
...
we need two cohorts: a target cohort (i.e. the population we’re
...
Now, we need to create the settings for the prediction model. First we
set up the database and cohort details.
...
Next, we create covariates using Feature Extraction as shown in vignette
6. This may take some time to run.
...
So we don’t have to run the above again, we save plpData as an object
...
Next, we set our population settings, choosing which persons in our
cohort to include/remove from the prediction.
...
Here,we select the type of prediction model we want to use. In this
example, we will use lasso logistic regression.

``` r
lr_model <- PatientLevelPrediction::setLassoLogisticRegression()
```

Now, we’re ready to run the model (and set a few more settings).
...
restrictive cohorts or edit any of the prediction settings.
...
Again, because the above code takes so long to run, let’s save the
results as an object for quick loading.
...
Now that we have run the prediction model, let’s view the results in two
of ways. First, to launch the R Shiny application, we run the following
code.
...
Finally, let’s view the results in tabular form. Once you run the
