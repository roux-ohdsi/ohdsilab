...
<https://ohdsi.github.io/CohortExplorer/>. This package retrieves
person-level data for persons included in a specific cohort and displays
it in a user-friendly Shiny application. Because this package displays
person-level data, any outputs should not be shared with individuals who
are not active OHDSI Lab Users (with the exception of outputs based on
...
The above code will generate several files in the export folder you
designated. Navigate to that folder and open the README.md file. Follow
the contained instructions to launch the Shiny application.

Note: If you run into errors when running renv::restore() within the
CohortExplorer R project, run the following code instead, then move on
to runApp().
