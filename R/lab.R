...
lab_set_password <- function() {
  keyring::key_set("db_username", prompt = "Username for this workspace")
  keyring::key_set("db_password", prompt = "Password for this workspace")
}

lab_get_password <- function() {
  keyring::key_get("db_password")
}
...
lab_authorize_atlas <- function() {
  ROhdsiWebApi::authorizeWebApi(lab_atlas_url(),
                                authMethod = "db",
                                webApiUsername = keyring::key_get("db_username"),
                                webApiPassword = keyring::key_get("db_password"))
}
...
lab_write_schema <- function() {
  paste0("work_", keyring::key_get("db_username"))
}
