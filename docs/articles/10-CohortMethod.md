...
<https://ohdsi.github.io/CohortMethod/>. Researchers can use this
package to calculate difference of risk of specific clinical outcomes
between a target cohort and a comparator cohort.
...
We’ll generate a cohort from ATLAS as we did in vignette 3, except now
we need three cohorts: a target cohort (i.e. the population we’re
...
we’re calculating risk for): 4861 (Hypoglycemia).
...
Now we’re ready to start desiging the CohortMethod analysis. First, we
create covariates using Feature Extraction as shown in vignette 6,
...
Because the extract takes so long, I recommend saving it in case you
need to come back to the later steps during another session. Once you
have saved the data, reload it as the same variable.
...
Create the study population with the appropriate risk windows. These can
be changed to match the needs of your study.
...
View attrition (how many subjects were excluded after each bit of
inclusion criteria?):
...
Run the propensity score model (setting cross-validation to FALSE saves
a lot of time, but this will still take a while):
...
Because the propensity score model took so long to run, save and reload
it.
...
Evaluate the propensity score model through the following 3 tests: 1.
...
stratifyByPs methodbut you could also use matchOnPs or
...
The next four steps evaluate the balance of your covariates. 1.
...
2.  Visualize the covariate balance using a scatter plot and a plot of
    the most prevalent covariates.
...
3.  Create a table 1 of covariates comparing the study population before
...
4.  See how different the adjusted study population is from the original
    study population by checking generalizability.

``` r
getGeneralizabilityTable(balance)
```

The next two steps provide and assessment of the follow-up and the
...
Finally, you’re ready to run the outcome model to calculate the
...
Now that you have your CohortMethod results, you can visualize them in a
variety of ways. 1. Create Kaplan-Meier plot:
...
That’s it! Obviously, many of the packages settings can be adjusted to
fit your study needs. The above just provides an example of what a
CohortMethod analysis might look like when run against OHDSI Lab data.
